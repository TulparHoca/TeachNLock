...
export default function TeacherToolbar() {
  // 🔥 teacherName ve scheduleStatus'u çektik
  const { lock, files, announcement, teacherName, scheduleStatus } = useLock();
  ...
  useEffect(() => {
    const t = setInterval(() => setSecondsLeft(p => p <= 0 ? 0 : p - 1), 1000);
  ...
  const handlePointerDown = (e: React.PointerEvent) => { if (e.button !== 0) return; isDragging.current = true; hasMoved.current = false; dragStart.current = { x: e.clientX, y: e.clientY }; initialPos.current = { ...miniPos }; (e.target as Element).setPointerCapture(e.pointerId); };
  const handlePointerMove = (e: React.PointerEvent) => { if (!isDragging.current) return; const dx = e.clientX - dragStart.current.x; const dy = e.clientY - dragStart.current.y; if (Math.abs(dx) > 5 || Math.abs(dy) > 5) hasMoved.current = true; setMiniPos({ x: initialPos.current.x + dx, y: initialPos.current.y + dy }); };
  const handlePointerUp = (e: React.PointerEvent) => { isDragging.current = false; (e.target as Element).releasePointerCapture(e.pointerId); if (!hasMoved.current) setViewMode('NORMAL'); };

  const getFileIcon = (type: string) => { if (type.includes('image')) return <Image size={20} className="text-purple-400"/>; if (type.includes('video')) return <Video size={20} className="text-red-400"/>; if (type === 'link') return <ExternalLink size={20} className="text-blue-400"/>; return <FileText size={20} className="text-cyan-400"/>; };
  const handleFileClick = (file: any) => { if (!file || !file.file_url) return; if (file.file_type.includes('image') || file.file_type.includes('video')) { setSelectedMedia(file); window.electron?.setIgnoreMouse(false); } else { let targetUrl = file.file_url.trim(); if (!targetUrl.startsWith('http')) targetUrl = 'https://' + targetUrl; window.electron?.openExternal(targetUrl); } };
        ...
        <div className={`h-[180px] flex flex-col px-4 pb-4 justify-between shrink-0 transition-all ${announcement ? 'pt-12' : 'pt-4'}`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 select-none">
               ...
               <div>
                   {/* 🔥 İSİM ve DERS BİLGİSİ GÖSTERİMİ */}
                   <h2 className="font-bold text-lg leading-none">{teacherName || 'TeachNlock'}</h2>
                   <span className="text-xs text-green-400 font-mono tracking-wide mt-1 block">
                       {scheduleStatus || 'Yükleniyor...'}
                   </span>
               </div>
            </div>
